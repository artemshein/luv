local TestCase, Factory, Mysql = require"TestCase", require"Database.Factory", require"Database.Mysql"
local Debug, io, tostring = require"Debug", io, tostring

module(...)

local MysqlTest = TestCase:extend{
	dsn = "mysql://test:test@localhost/test",
	setUp = function (self)
		self.db = Factory:connect(self.dsn)
	end,
	testClasses = function (self)
		local db = self.db
		self.assertTrue(db:select():isKindOf(db.Select))
		self.assertTrue(db:selectRow():isKindOf(db.SelectRow))
		self.assertTrue(db:selectCell():isKindOf(db.SelectCell))
		self.assertTrue(db:insert():isKindOf(db.Insert))
		self.assertTrue(db:insertRow():isKindOf(db.InsertRow))
		self.assertTrue(db:update():isKindOf(db.Update))
		self.assertTrue(db:updateRow():isKindOf(db.UpdateRow))
		self.assertTrue(db:delete():isKindOf(db.Delete))
		self.assertTrue(db:deleteRow():isKindOf(db.DeleteRow))
		self.assertTrue(db:createTable():isKindOf(db.CreateTable))
		self.assertTrue(db:dropTable():isKindOf(db.DropTable))
	end,
